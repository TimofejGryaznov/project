import tkinter as tk  # Импорт модуля tkinter для создания графического интерфейса пользователя
from tkinter import ttk # Импорт подмодуля ttk для создания расширенных виджетов
import sqlite3 # Импорт модуля sqlite3 для работы с базой данных

class Main(tk.Frame): # Создание класса Main, наследующего свойства класса tk.Frame
    ...
    def init_main(self): # Определение метода init_main
    ...
    def search_records(self, name): # Определение метода search_records
        name = ('%' + name + '%',)
        self.db.c.execute(
    ...
    def open_dialog(self): # Определение метода open_dialog # Определение метода view_records
        Child()

    def records(self, name, tel, email): # Определение метода records
        self.db.insert_data(name, tel, email)
        self.view_records()

    def view_records(self): # Определение метода view_records
        self.db.c.execute('''SELECT * FROM db''')
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db.c.fetchall()]

    def open_update_dialog(self): # Определение метода open_update_dialog
        Update()

    def update_records(self, name, tel, email): # Определение метода update_records
